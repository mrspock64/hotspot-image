...
#
# This is stock SvxLink's own Logic.tcl (as shipped on this hardware,
# confirmed against /usr/share/svxlink/events.d/Logic.tcl.bak-20260901-223010),
# NOT a from-scratch replacement. An earlier hotspot-image version *did* ship a
# minimal from-scratch file here, on the assumption that a separate
# events.d/local/Logic.tcl override would layer on top of this file without
# touching it. That assumption was wrong: on this image, events.d/local is a
# symlink to events.d itself (`local -> .`), not a real subdirectory, so
# installing to local/Logic.tcl overwrites this exact file -- silently
# deleting manual_identification (the "*" DTMF re-identification announcement),
# checkPeriodicIdentify, startup, and everything else below. Going forward,
...
#   - D920/D921: hotspot-image's own custom/alert message slots -- play a
#     pre-generated TTS file if one has been saved via the dashboard's Radio
#     Test page (dashboard/include/tts_message.php). See events.d/Logic.tcl
#     in the hotspot-image repo for the canonical source of this file.
#
###############################################################################
...
# Fix for https://github.com/Guru-RF/Analog-HotSPOT-SVXLink/issues/2 --
# when this Logic.tcl is used from SimplexLogicType.tcl/RepeaterLogicType.tcl
# (as it is here: [SimplexLogic] TYPE=Simplex) rather than LogicBase.tcl,
# locale.tcl (spellWord, playTime, playNumber, playFrequency, ...) is never
# sourced into this namespace and the namespace path is never extended to the
# parent, so every locale-dependent proc below throws "invalid command name"
# at runtime. Mirrors what LogicBase.tcl does for ReflectorLogic.
...
proc dtmf_cmd_received {cmd} {
  ...
  # D911: reads out the hotspot's current LAN IP address, digit by digit,
  # over the air -- handy for a headless device whose WiFi network may have
  # changed. Error-handled so a lookup failure announces "operation failed"
  # instead of leaving a silent, half-finished transmission.
  if {$cmd == "D911"} {
    if {[catch {
  ...
  # together SvxLink's own per-word/per-digit sound library like D911# does
  # above -- SvxLink has no free-text speech of its own, so this is how the
  # dashboard's Radio Test page gets to transmit arbitrary custom content
  # (see dashboard/include/tts_message.php, which generates this file via
  # espeak-ng). Silently does nothing if the file doesn't exist yet -- no
  # custom message has been saved.
  if {$cmd == "D920"} {
