...
# Check FIRST, before touching apt/users/anything -- a from-source build
# (git clone + cmake + make) takes 20-40+ minutes on a Pi Zero 2 W and
# noticeably loads the box while SvxLink itself is still running live
# (see svxlinkuhf's own performance notes). Confirmed live: this script
# used to always rebuild unconditionally, even when already up to date,
# because the version-compare check.svxlink.sh already does was never
# also applied here before starting the heavy part.
...
if [ -n "$installed" ] && [ "$installed" = "$tagname" ]; then
  echo "Already up to date ($installed) -- nothing to build."
  echo "###-FINISH-####"
  exit 0
fi

echo "Installed: ${installed:-unknown}, latest: $tagname -- building."
