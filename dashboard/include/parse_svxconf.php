    ...
    if (fopen($svxConfigFile,'r'))
       {$svxconfig = parse_ini_file($svxConfigFile,true,INI_SCANNER_RAW);
         ...
         $check_logics = explode(",",$svxconfig['GLOBAL']['LOGICS']);
            ...
            // if we work with CTCSS please set REPORT_CTCSS with correct value in svxlink.conf
            $ctcss = $svxconfig['SimplexLogic']['REPORT_CTCSS'] ?? null;
            $system_type = "IS_SIMPLEX"; // if simplex
            $dtmfctrl = $svxconfig['SimplexLogic']['DTMF_CTRL_PTY'] ?? null;
        }
         }
         ...
         // TG_URI ??? I don't know...
         // All optional -- most nodes don't set these, so default rather than warn.
