...
<div class="mx-card">
  ...
  <table class="mx-table">
    ...
    <tr><td><b>Bluetooth</b></td><td>On/off switch for the BLE service the <a href="https://svxlink-hotspot.app" target="_blank" rel="noopener">companion app</a> (RF.Guru's own Analog-HotSPOT-App, linked from that portal) connects to, plus a "require pairing" setting. No pairing/password is required to connect by default, so turn it on only while actively using the app nearby -- it always starts back up OFF after a reboot, whatever state you leave it in here. Turning on "Require pairing for Bluetooth commands" instead forces the phone to bond first before it can send anything.
      <p class="mx-hint" style="margin:4px 0 0;">Known issue (2026-09-10): on newer BlueZ (5.66, current Debian bookworm), the service starts and reports advertising successfully but the app may never actually see the node -- confirmed live (<code>ActiveInstances: 0</code> despite a successful <code>btmgmt add-adv</code>). This is in RF.Guru's own fetched <code>hotspot-bluetooth</code> script (this project deliberately doesn't fork/maintain it), likely a BlueZ-version compatibility gap in how it registers the advertisement. Not yet fixed here.</p>
    </td></tr>
