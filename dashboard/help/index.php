...
<div class="mx-card">
  ...
  <table class="mx-table">
    ...
    <tr><td><b>Update</b></td><td>Check installed versions and trigger updates for the OS, SvxLink, and the dashboard itself -- each upgrade asks for confirmation first. The log updates live while an action runs (no need to keep reloading). Upgrading the OS also snapshots exactly which package versions were installed beforehand (in case anything needs comparing afterward) and cleans up old, no-longer-bootable kernel packages automatically. A kernel/firmware upgrade needs a device restart (Power page) afterward to fully take effect. The header's "Dashboard update" badge only re-checks every 6 hours to avoid hitting GitHub on every page load -- click Check/Update here directly if you don't want to wait for it.
      <p class="mx-hint" style="margin:4px 0 0;">The dashboard itself also auto-updates on its own -- roughly hourly, on by default -- applying a new commit the same way the "Update Dashboard" button does (backup, re-sync, restart SvxLink). Toggle it off in the "Auto-update" section further down this page. Deliberately scoped to the dashboard only, never OS or SvxLink -- those need a confirmed click every time, they're too invasive (a kernel upgrade needing a reboot, an SvxLink rebuild taking the better part of an hour) to ever apply unattended.</p>
      <p class="mx-hint" style="margin:4px 0 0;">Whenever a dashboard update actually applies (manual click or the hourly auto-update), a green pulsing "&#10003; Updated" badge appears in the header for 24 hours -- so a change that happened unattended, overnight, is still noticeable the next time someone looks. The Update page itself also keeps a permanent "Last update applied" line under the title regardless of how long ago it was.</p>
    </td></tr>
