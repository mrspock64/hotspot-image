...
#
# Opt-in only -- setup.sh calls this when run with --with-dashboard-v2,
# never by default. dashboard-v2 is still under active development (real
# bugs found and fixed most days -- see docs/dashboard-v2-brief.md), so a
# plain `setup.sh` on a fresh node stays exactly what it always was: just
# the stable production dashboard. This script itself can still be run
# by hand any time, on any node, independent of setup.sh.
#
# Idempotent: safe to re-run (also how svxlinkuhf picks up a newer
# dashboard-v2 commit -- re-run this, or `git -C
