...
- **RX Monitor-modulen kan återanvända "monitor-only"-mönstret** — produktionens `dashboard/include/rx_monitor_toggle.php` (main-branchen, inte här) löser exakt samma problem RX Monitor-modulen i v2 skulle ha: låt knappen slå på QSO Recorder tillfälligt utan att permanent logga, kasta inspelningen via en flagga `tag_and_encode.py` kollar (`/dev/shm/hotspot_rx_monitor_only`), stäng av igen efter en kort fördröjning. Samma mönster (start/stop-endpoint + flagg-fil + fördröjd avstängning som respekterar om riktig loggning slagits på under tiden) kan återanvändas rakt av i `dashboard-v2/api/rxmonitor.php` om/när RX Monitor-modulen ska funka utan att QSO Log är påslaget.
