...
- **Hantera TG-listan (edit mode)**: pennikon i Monitored Talkgroups panelhuvud. Namnbyte/ny TG sparas direkt per fält (`?action=rename`, ren JSON-write, ingen omstart). Monitor på/av + prioritet samlas ihop och sparas med en enda explicit "Save monitor list & restart SvxLink"-knapp (`?action=save_monitor`, återanvänder produktionens `saveMonitoredTgs()` — den enda v2-åtgärden som faktiskt startar om SvxLink, medvetet inte auto-save per kryssruta). Ersätter både produktionens "Talk Groups"-tabell och "TG Names"-sidan för det här nodens bruk. Ingen ta-bort-TG-funktion byggd än (utanför scope denna omgång).
