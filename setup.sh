...
echo "=== 2/9: persistent crash logging ==="
...
echo "=== 3/9: audio (WM8960) + GPIO PTT ==="
bash "$SCRIPT_DIR/lib/audio-gpio.sh"

echo "=== 4/9: building SvxLink (stock) + installing our Logic.tcl ==="
...
echo "=== 7/9: load monitor (load/iowait/memory) ==="
bash "$SCRIPT_DIR/lib/install-load-monitor.sh"

echo "=== 8/9: RX Monitor audio streaming + QSO Log ==="
bash "$SCRIPT_DIR/lib/install-rx-monitor.sh"

echo "=== 9/9: Bluetooth companion app support ==="
